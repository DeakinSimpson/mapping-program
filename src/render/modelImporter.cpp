...
Model load_land_polygons(const char *modelFilepath) {
    ...
    for (int i = 0; i < number_of_polygons; ++i) {
        ...
        int number_of_verticies = shp_polygon->nVertices;

        // loop through each verticie
        for (int j = 0; j < number_of_verticies; j++) {
            Vertex vertex;

            // extract the x and y data from shp_polygon
            vertex.x = (float)shp_polygon->padfX[j];
            vertex.y = (float)shp_polygon->padfY[j];
            vertex.z = 0.0f;

            // push the current vertex to teh polygon verticie
            polygon.verticies.push_back(vertex);
        }

        // push the current polygon to the land_polygons
